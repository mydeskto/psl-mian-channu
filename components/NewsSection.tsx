          ...
          {news.map((item, index) => (
            <article
              ...
              className={`group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card ${
                index === 0 ? 'lg:col-span-2 lg:grid lg:grid-cols-[1fr_1.2fr]' : ''
              }`}
            >
              ...
              <div className="p-6">
                <p className="text-[10px] font-bold uppercase tracking-widest text-brandGreen">
                  {item.category} · {item.date}
                </p>
                <h3 className="display mt-2 text-3xl font-bold uppercase leading-none text-navy">
                  {item.title}
                </h3>
                <p className="mt-3 text-sm leading-6 text-slate-500">{item.summary}</p>
                <Link
                  href={`/news/${item.id}`}
                  className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-navy"
                >
